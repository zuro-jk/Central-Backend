package com.centrral.centralres.features.suppliers.service;
...
import com.centrral.centralres.features.suppliers.exceptions.PurchaseOrderNotFoundException;
...
import com.centrral.centralres.features.suppliers.model.PurchaseOrderDetail;
import com.centrral.centralres.features.suppliers.model.Supplier;
import com.centrral.centralres.features.suppliers.repository.PurchaseOrderRepository;
...
public class PurchaseOrderService {
    ...
    public PurchaseOrderResponse create(PurchaseOrderRequest request) {
        ...
        if (status == PurchaseOrderStatus.RECEIVED) {
            details.forEach(
                    d -> updateInventoryStock(d.getIngredient().getId(), BigDecimal.valueOf(d.getQuantity()), true));
        }
    ...
    public PurchaseOrderResponse update(Long id, PurchaseOrderRequest request) {
        ...
        if (oldStatus == PurchaseOrderStatus.RECEIVED && newStatus != PurchaseOrderStatus.RECEIVED) {
            order.getDetails().forEach(
                    d -> updateInventoryStock(d.getIngredient().getId(), BigDecimal.valueOf(d.getQuantity()), false));
        }
        ...
        request.getDetails().forEach(d -> {
            Ingredient ingredient = ingredientRepository.findById(d.getIngredientId())
                    .orElseThrow(() -> new IngredientNotFoundException(
                            "Ingrediente no encontrado con id: " + d.getIngredientId()));
